      ...
      <div className="w-full max-w-md bg-[#161b22] p-7 rounded-2xl border border-[#30363d] shadow-2xl relative">
        ...
        <div className="flex items-center gap-3 mb-5">
          ...
          <div>
            ...
            <p className="text-[11px] text-[#8b949e]">Updates for everyone in your squad</p>
          </div>
        </div>

        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-xs font-semibold text-[#8b949e] uppercase tracking-wider mb-2">Questions Per Day</label>
            <input type="number" min="1" max="100" required value={targetValue} onChange={(e) => setTargetValue(e.target.value)}
              className="w-full px-4 py-3 bg-[#0d1117] border border-[#30363d] rounded-xl text-white font-extrabold text-2xl text-center focus:outline-none focus:ring-2 focus:ring-[#22c55e] focus:border-transparent"
            />
          </div>
          {error && <div className="p-2.5 rounded-lg bg-red-500/10 border border-red-500/25 text-red-400 text-xs text-center">{error}</div>}
          <div className="flex items-center justify-end gap-2.5 pt-3 border-t border-[#21262d]">
            <button type="button" onClick={onClose} className="px-4 py-2 bg-[#21262d] hover:bg-[#30363d] text-[#8b949e] rounded-xl text-xs font-semibold transition-colors">Cancel</button>
