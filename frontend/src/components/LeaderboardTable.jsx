...
const LeaderboardTable = ({
  ...
  return (
    <div className="dash-card overflow-hidden relative">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 px-4 sm:px-5 py-4 border-b border-[#222225] dark:border-[#222225] light:border-slate-200">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-[11px] text-[#A1A1AA] dark:text-[#A1A1AA] light:text-slate-500">Compete with friends to hit 5 a day</p>
          </div>
        </div>

        {/* Filter Tabs */}
        <div className="flex items-center bg-[#09090B] dark:bg-[#09090B] light:bg-slate-100 p-1 rounded-xl border border-[#222225] dark:border-[#222225] light:border-slate-200 self-start sm:self-auto shadow-sm">
          <button
            onClick={() => setViewMode('today')}
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${
              viewMode === 'today'
                ? 'bg-[#EA5D3A] text-white shadow-md'
                : 'text-[#A1A1AA] dark:text-[#A1A1AA] light:text-slate-600 hover:text-white dark:hover:text-white light:hover:text-slate-900'
            }`}
          >
      ...
      {/* 👑 Top 3 Podium Cards */}
      {topThree.length > 0 && (
        <div className="p-4 bg-gradient-to-b from-[#121215]/70 to-[#09090B]/40 dark:from-[#121215]/70 dark:to-[#09090B]/40 light:from-slate-100/80 light:to-slate-50/50 border-b border-[#222225] dark:border-[#222225] light:border-slate-200">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            {topThree.map((user, idx) => {
              ...
              const cardBorder = isGold
                ? 'border-amber-500/40 dark:border-amber-500/40 light:border-amber-400 bg-gradient-to-b from-amber-500/10 dark:from-amber-500/10 light:from-amber-100/50 to-transparent'
                : isSilver
                ? 'border-slate-400/30 dark:border-slate-400/30 light:border-slate-300 bg-gradient-to-b from-slate-400/10 dark:from-slate-400/10 light:from-slate-200/50 to-transparent'
                : 'border-amber-700/30 dark:border-amber-700/30 light:border-amber-600/30 bg-gradient-to-b from-amber-700/10 dark:from-amber-700/10 light:from-amber-100/30 to-transparent';
              ...
              return (
                <div
                  key={user.id}
                  className={`p-3.5 rounded-2xl border ${cardBorder} flex items-center justify-between relative overflow-hidden transition-all hover:scale-[1.02] shadow-sm`}
                >
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      <div className="flex items-center gap-1">
                        <span className="text-xs font-extrabold text-white dark:text-white light:text-slate-900 truncate">{user.name}</span>
                        {user.isSelf && (
                          <span className="px-1 py-0.2 rounded text-[8px] font-bold bg-[#EA5D3A]/20 text-[#EA5D3A]">You</span>
                        )}
                      </div>
                      <p className="text-[10px] text-[#A1A1AA] dark:text-[#A1A1AA] light:text-slate-500">@{user.leetcodeUsername || 'grinder'}</p>
                    </div>
                  </div>
                  ...
                  <div className="text-right flex-shrink-0">
                    ...
                    <span className="text-[9px] font-bold text-[#A1A1AA] dark:text-[#A1A1AA] light:text-slate-500">
      ...
      {/* Table Header */}
      <div className="grid grid-cols-[50px_1fr_80px_100px] sm:grid-cols-[50px_1fr_90px_1fr_90px_110px] items-center px-4 sm:px-5 py-2.5 text-[10px] sm:text-[11px] font-bold text-[#71717A] dark:text-[#71717A] light:text-slate-500 uppercase tracking-wider border-b border-[#222225]/60 dark:border-[#222225]/60 light:border-slate-200 bg-[#09090B]/30 dark:bg-[#09090B]/30 light:bg-slate-50">
        <span>Rank</span>
      ...
      {/* Rows */}
      <div className="divide-y divide-[#222225]/50 dark:divide-[#222225]/50 light:divide-slate-200">
              ...
              >
                ...
                <div className="flex items-center gap-2.5 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-[10px] text-[#A1A1AA] dark:text-[#A1A1AA] light:text-slate-500 truncate">@{user.leetcodeUsername || 'leetcode'}</p>
                  </div>
                </div>
                ...
                <div className="hidden sm:block px-2">
                  <div className="flex items-center gap-2.5">
                    <span className={`text-xs font-bold font-mono min-w-[36px] ${isTargetHit ? 'text-[#EA5D3A]' : 'text-[#A1A1AA] dark:text-[#A1A1AA] light:text-slate-500'}`}>
                      {progressPercent}%
                    </span>
      ...
      {/* Footer */}
      <div className="px-5 py-3 border-t border-[#222225] dark:border-[#222225] light:border-slate-200 flex items-center justify-between text-xs text-[#A1A1AA] dark:text-[#A1A1AA] light:text-slate-500">
