...
const Navbar = ({ onToggleSidebar, onToggleCollapse, isCollapsed, onRefresh, refreshing, platformTotal = 0 }) => {
  ...
  return (
    <header className="h-16 flex items-center justify-between px-4 md:px-6 border-b border-[#27272A] dark:border-[#27272A] light:border-slate-200 bg-[#09090B] dark:bg-[#09090B] light:bg-white flex-shrink-0">
      {/* Left side: Primary Logo + Collapse Button + Mobile Menu */}
      <div className="flex items-center gap-3">
        <button
          onClick={onToggleSidebar}
          className="lg:hidden p-2 rounded-xl text-[#A1A1AA] dark:text-[#A1A1AA] light:text-slate-600 hover:text-white dark:hover:text-white light:hover:text-slate-900 hover:bg-white/5 dark:hover:bg-white/5 light:hover:bg-slate-100 transition-colors"
        ...
        <div className="flex items-center gap-2 cursor-pointer" onClick={() => navigate('/')}>
          <img src="/logo.png" alt="GrindFam Logo" className="w-8 h-8 rounded-lg object-cover border border-[#27272A] dark:border-[#27272A] light:border-slate-200 flex-shrink-0" />
      ...
      <div className="relative flex-1 max-w-md mx-4" ref={searchRef}>
        <div className="relative flex items-center">
          <Search className="absolute left-3 w-4 h-4 text-[#71717A] dark:text-[#71717A] light:text-slate-400" />
          <input
            ref={inputRef}
            type="text"
            value={searchQuery}
            ...
            className="w-full pl-9 pr-8 py-1.5 bg-[#121215] dark:bg-[#121215] light:bg-slate-50 border border-[#27272A] dark:border-[#27272A] light:border-slate-200 rounded-xl text-xs text-[#F4F4F5] dark:text-[#F4F4F5] light:text-slate-900 placeholder-[#71717A] dark:placeholder-[#71717A] light:placeholder-slate-400 focus:outline-none focus:border-[#EA5D3A] transition-all shadow-sm"
          />
          {searchQuery ? (
            <button
            ...
            <span className="absolute right-2 px-1.5 py-0.5 bg-[#222225] dark:bg-[#222225] light:bg-slate-200 border border-[#27272A] dark:border-[#27272A] light:border-slate-300 rounded text-[9px] text-[#71717A] dark:text-[#71717A] light:text-slate-600 font-mono hidden sm:inline-block">
              Ctrl+K
            </span>
          )}
        </div>
        ...
        <AnimatePresence>
          {isSearchOpen && (searchResults.length > 0 || topicResults.length > 0 || searchLoading) && (
            <motion.div
              ...
              className="absolute top-full left-0 mt-1.5 w-full min-w-[320px] max-w-[420px] bg-[#121215] dark:bg-[#121215] light:bg-white border border-[#27272A] dark:border-[#27272A] light:border-slate-200 rounded-xl shadow-2xl overflow-hidden z-50"
            >
              {searchLoading ? (
                <div className="p-4 text-center">
                  <div className="w-4 h-4 border-2 border-[#EA5D3A] border-t-transparent rounded-full animate-spin mx-auto mb-1" />
                  <span className="text-xs text-[#A1A1AA] dark:text-[#A1A1AA] light:text-slate-500">Searching...</span>
                </div>
              ) : (
                <>
                  {topicResults.length > 0 && (
                    <div className="px-3 py-2.5 border-b border-[#222225] dark:border-[#222225] light:border-slate-200">
                      <p className="text-[9px] font-bold text-[#71717A] dark:text-[#71717A] light:text-slate-500 uppercase tracking-widest mb-2">Topics</p>
                  ...
                  {searchResults.length > 0 && (
                    <div className="max-h-[320px] overflow-y-auto">
                      <p className="px-3 pt-2.5 pb-1 text-[9px] font-bold text-[#71717A] dark:text-[#71717A] light:text-slate-500 uppercase tracking-widest">
                      ...
                      {searchResults.map((result, idx) => (
                        <div
                          ...
                          className="flex items-center justify-between gap-3 px-3 py-2 hover:bg-[#09090B] dark:hover:bg-[#09090B] light:hover:bg-slate-100 cursor-pointer transition-colors border-b border-[#222225]/50 dark:border-[#222225]/50 light:border-slate-200 last:border-b-0"
                        >
                          <div className="flex items-center gap-2.5 min-w-0">
                            <div className="w-6 h-6 rounded-md flex items-center justify-center flex-shrink-0 bg-[#181818] dark:bg-[#181818] light:bg-slate-100 border border-white/10 dark:border-white/10 light:border-slate-200 text-[#A1A1AA]">
                              {result.source_type === 'company'
                                ? <Building2 className="w-3 h-3 text-[#A1A1AA] dark:text-[#A1A1AA] light:text-slate-600" />
                            ...
                            <div className="min-w-0">
                              ...
                              {result.source_name && (
                                <p className="text-[9px] text-[#71717A] dark:text-[#71717A] light:text-slate-500 truncate">from {result.source_name}</p>
                              )}
                            </div>
                          </div>
                  ...
                  {searchResults.length === 0 && topicResults.length === 0 && searchQuery.length >= 2 && (
                    <div className="py-6 text-center">
                      <p className="text-xs text-[#71717A] dark:text-[#71717A] light:text-slate-500">No results found for "{searchQuery}"</p>
                    </div>
                  )}
                </>
              )}
      ...
      <div className="flex items-center gap-2 sm:gap-3 ml-3">
        ...
        <div className="hidden md:flex items-center gap-1.5 px-3 h-8 rounded-full bg-[#121215] dark:bg-[#121215] light:bg-slate-50 border border-[#27272A] dark:border-[#27272A] light:border-slate-200 text-xs font-medium text-zinc-400 dark:text-zinc-400 light:text-slate-600 shadow-sm">
          <Calendar className="w-3.5 h-3.5" />
          <span>{dateStr}</span>
        </div>

        <button className="relative p-2 rounded-xl hover:bg-white/5 dark:hover:bg-white/5 light:hover:bg-slate-100 text-[#A1A1AA] dark:text-[#A1A1AA] light:text-slate-600 transition-colors">
        ...
        <button
          onClick={toggleTheme}
          className="p-2 rounded-xl border border-zinc-800/80 dark:border-zinc-800/80 light:border-slate-200 bg-[#121215] dark:bg-[#121215] light:bg-slate-50 text-[#A1A1AA] dark:text-[#A1A1AA] light:text-slate-600 hover:text-amber-400 hover:border-amber-500/40 transition-all flex items-center justify-center relative group shadow-sm"
