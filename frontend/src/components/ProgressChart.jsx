...
const ProgressChart = ({ yourTodayCount = 0, dailyTarget = 5, weeklyData = [] }) => {
  ...
  return (
    <div className="dash-card overflow-hidden">
      <div className="px-5 py-4 border-b border-[#222225]">
      ...
      <div className="p-5 flex flex-col sm:flex-row gap-6">
        ...
        <div className="flex-1 min-w-0">
          <div className="relative h-36">
            {/* Y-axis labels */}
            <div className="absolute left-0 top-0 bottom-5 flex flex-col justify-between text-[10px] text-[#71717A] font-medium w-8">
                ...
                return (
                  <div key={day} className="flex-1 flex flex-col items-center gap-1.5 h-full justify-end">
                    ...
                    <div className="w-full flex items-end relative" style={{ height: '75%' }}>
                      ...
                      {val > 0 && (
                        <div
                          className="absolute left-1/2 -translate-x-1/2 w-2.5 h-2.5 rounded-full bg-[#22c55e] border-2 border-[#121215]"
                          style={{ bottom: `${height}%`, transform: 'translate(-50%, 50%)' }}
                        ></div>
                      )}
                    </div>
                    <span className={`text-[10px] ${isToday ? 'text-[#22c55e] font-bold' : 'text-[#71717A]'}`}>{day}</span>
                  </div>
                );
              })}
            </div>
        ...
        <div className="flex flex-row sm:flex-col items-center justify-center gap-4 sm:gap-3 sm:w-40 flex-shrink-0">
          ...
          <div className="relative w-20 h-20 sm:w-24 sm:h-24 flex-shrink-0">
            <svg viewBox="0 0 100 100" className="w-full h-full -rotate-90">
              <circle cx="50" cy="50" r={radius} fill="none" stroke="#222225" strokeWidth="8" />
              <circle
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              ...
              <span className="text-[10px] text-[#A1A1AA] font-medium">Today</span>
            </div>
          </div>
          {/* Legend */}
          <div className="space-y-1.5 text-xs">
            ...
            <div className="flex items-center gap-2">
              <span className="w-2.5 h-2.5 rounded-full bg-[#222225] flex-shrink-0"></span>
              <span className="text-[#A1A1AA] font-medium">{remaining} Remaining</span>
            </div>
            <p className="text-[10px] text-[#71717A] pt-1">Daily Target: {dailyTarget}</p>
          </div>
        </div>
      </div>
    </div>
