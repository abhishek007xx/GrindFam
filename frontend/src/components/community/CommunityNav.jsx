...
import { Flame, Swords, Users, Sparkles, ShieldCheck } from 'lucide-react';
...
export function CommunityNav({ activeMode, onSelectMode, streakCount = 14, isShieldActive = true }) {
  const modes = [
    ...
    { id: 'squad', label: 'Squad & Peer Hub', icon: Users, badge: null, ariaControl: 'squad-panel' }
  ];
    ...
    >
      ...
      <div className="flex items-center gap-3 w-full sm:w-auto justify-end border-t sm:border-t-0 border-[#21262D] pt-2 sm:pt-0">
        <div
          ...
          className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-[#EA5D3A]/15 border border-[#EA5D3A]/30 text-[#EA5D3A] text-xs font-bold shadow-sm"
        >
          <Flame className="w-3.5 h-3.5 fill-current animate-pulse" aria-hidden="true" />
