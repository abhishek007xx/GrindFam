...
export default function StitchSettings() {
  ...
  const { activeSquad, updateSquadSettings, leaveSquad, deleteSquad, members } = useSquadStore();

  const [activeTab, setActiveTab] = useState('account');
  const [isSaving, setIsSaving] = useState(false);
  const [toast, setToast] = useState(null);
  const [showConfirmModal, setShowConfirmModal] = useState(null); // 'leave' | 'delete'
  ...
  // Populate data on mount or state change
  useEffect(() => {
    if (profile || user) {
  ...
  const handleSaveAccount = async (e) => {
    ...
    try {
      ...
      // 2. Update Profiles Table (graceful update)
      try {
        await supabase
          .from('profiles')
          .update({
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-md z-[110] flex items-center justify-center p-4">
          <div className="bg-[#1c1b1b] border border-[#59413b]/60 p-6 rounded-2xl max-w-md w-full shadow-2xl space-y-4">
            <h3 className="font-['Outfit'] text-2xl font-bold text-[#e5e2e1]">
              {showConfirmModal === 'leave' ? 'Leave Squad?' : 'Delete Squad?'}
            </h3>
            <p className="font-['Inter'] text-sm text-[#e1bfb7]">
              {showConfirmModal === 'leave'
                ? 'Are you sure you want to leave this squad? You will lose access to all squad chat and resources.'
                : 'Are you sure you want to permanently delete this squad? All messages, channels, and snippets will be erased.'}
            </p>
            <div className="flex justify-end gap-3 pt-2">
              ...
              <button 
                onClick={showConfirmModal === 'leave' ? handleLeaveSquadConfirm : handleDeleteSquadConfirm}
                disabled={isSaving}
              ...
              >
                {isSaving ? 'Processing...' : showConfirmModal === 'leave' ? 'Yes, Leave' : 'Yes, Delete'}
              </button>
            </div>
          </div>
        </div>
      ...
      <aside className="w-full md:w-[240px] flex-shrink-0">
        ...
        <nav className="flex flex-col gap-2">
          <button 
            ...
            className={`w-full text-left px-4 py-3 rounded font-medium transition-colors ${
              activeTab === 'account' 
              ...
              {(activeSquad?.invite_code || activeSquad?.code) && (
                <div className="p-4 rounded-xl bg-[#201f1f] border border-[#59413b]/40 flex items-center justify-between">
                  ...
                  <button 
                    ...
                    className="px-4 py-2 rounded-lg bg-[#4cd7f6]/10 border border-[#4cd7f6]/40 text-[#4cd7f6] hover:bg-[#4cd7f6]/20 transition-all font-['JetBrains_Mono'] text-xs font-bold flex items-center gap-1.5"
                  >
