  ...
  return (
    <div className="flex items-center gap-2 mt-2 pt-2 border-t border-[#333333]">
      <input
        ...
        className="flex-1 px-3 py-1.5 bg-[#141414] border border-[#333333] rounded-md text-xs text-[#F4F4F5] placeholder-[#6B7280] focus:outline-none focus:border-[#EA5D3A] transition-all"
      />
      ...
      {editing && !previewMode ? (
        <div className="space-y-2.5">
          <input value={project.title} onChange={e => onUpdate({ title: e.target.value })} placeholder="Project title"
            className="w-full px-3 py-1.5 bg-[#141414] border border-[#333333] rounded-md text-xs text-[#F4F4F5] placeholder-[#6B7280] focus:outline-none focus:border-[#EA5D3A] transition-all font-semibold" />
          <textarea value={project.description} onChange={e => onUpdate({ description: e.target.value })} placeholder="What does this project do?" rows={3}
            className="w-full px-3 py-1.5 bg-[#141414] border border-[#333333] rounded-md text-xs text-[#F4F4F5] placeholder-[#6B7280] focus:outline-none focus:border-[#EA5D3A] transition-all resize-none" />
          ...
          <div className="flex gap-2">
            <input value={newTech} onChange={e => setNewTech(e.target.value)}
              ...
              placeholder="Add tech (press Enter)" className="flex-1 px-3 py-1.5 bg-[#141414] border border-[#333333] rounded-md text-xs text-[#F4F4F5] placeholder-[#6B7280] focus:outline-none focus:border-[#EA5D3A] transition-all" />
            <select value={project.status} onChange={e => onUpdate({ status: e.target.value })}
              className="px-2 py-1.5 bg-[#141414] border border-[#333333] rounded-md text-xs text-[#F4F4F5] focus:outline-none focus:border-[#EA5D3A]">
        ...
        <>
          <div className="flex items-start justify-between gap-2">
            ...
            {!previewMode && (
              <div className="flex gap-1 flex-shrink-0">
                <button onClick={() => setEditing(true)} className="p-1.5 rounded-md bg-[#2C2C2C] text-[#9CA3AF] hover:text-white transition-colors"><Pencil className="w-3.5 h-3.5" /></button>
                <button onClick={onRemove} className="p-1.5 rounded-md bg-[#2C2C2C] text-[#9CA3AF] hover:text-rose-400 transition-colors"><Trash2 className="w-3.5 h-3.5" /></button>
              </div>
            )}
          </div>
          <div className="flex flex-wrap gap-1.5">
          ...
          <div className="flex items-center gap-3 pt-1 border-t border-[#333333]/60">
    ...
    <div className="bg-[#1E1E1E] border border-[#333333] rounded-lg p-4">
      {editing && !previewMode ? (
        <div className="space-y-2.5">
          <div className="grid grid-cols-2 gap-2">
            <input value={exp.role} onChange={e => onUpdate({ role: e.target.value })} placeholder="Role / Title"
              className="px-3 py-1.5 bg-[#141414] border border-[#333333] rounded-md text-xs text-[#F4F4F5] placeholder-[#6B7280] focus:outline-none focus:border-[#EA5D3A] transition-all" />
          ...
          <input value={exp.duration} onChange={e => onUpdate({ duration: e.target.value })} placeholder="Duration (e.g. Jun 2024 - Present)"
            className="w-full px-3 py-1.5 bg-[#141414] border border-[#333333] rounded-md text-xs text-[#F4F4F5] placeholder-[#6B7280] focus:outline-none focus:border-[#EA5D3A] transition-all" />
        ...
        <div className="flex gap-3">
          <div className="w-9 h-9 rounded-lg bg-[#2C2C2C] border border-[#333333] flex items-center justify-center flex-shrink-0">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-start justify-between gap-2">
              ...
              {!previewMode && (
                <div className="flex gap-1">
                  <button onClick={() => setEditing(true)} className="p-1 rounded-md bg-[#2C2C2C] text-[#9CA3AF] hover:text-white transition-colors"><Pencil className="w-3 h-3" /></button>
                  <button onClick={onRemove} className="p-1 rounded-md bg-[#2C2C2C] text-[#9CA3AF] hover:text-rose-400 transition-colors"><Trash2 className="w-3 h-3" /></button>
                </div>
              )}
            </div>
    ...
    <div className="bg-[#1E1E1E] border border-[#333333] rounded-lg p-4">
      {editing && !previewMode ? (
        ...
        <>
          ...
          <div className="flex items-center justify-between mt-2 pt-2 border-t border-[#333333]/60">
  ...
  return (
    <div className="space-y-6 animate-fadeIn pb-12">
      ...
      <div className="relative overflow-hidden rounded-lg bg-[#1E1E1E] border border-[#333333] p-6 md:p-8">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <button onClick={() => setPreviewMode(!previewMode)}
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs font-medium border transition-all ${previewMode ? 'bg-emerald-500/15 text-emerald-400 border-emerald-500/30' : 'bg-[#262626] text-[#9CA3AF] border-[#333333] hover:text-white'}`}>
            ...
            <button onClick={() => setDraft(d => ({ ...d, isPublic: !d.isPublic }))}
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs font-medium border transition-all ${draft.isPublic ? 'bg-blue-500/15 text-blue-400 border-blue-500/30' : 'bg-[#262626] text-[#9CA3AF] border-[#333333] hover:text-white'}`}>
        ...
        <div className="relative z-10 flex flex-wrap items-center gap-4 mt-4 pt-4 border-t border-[#333333]/60">
          {[
      ...
      {/* Tabs */}
      <div className="flex items-center gap-1 overflow-x-auto scrollbar-none border-b border-[#333333]">
        {tabs.map(tab => (
        ...
        <motion.div key={activeTab} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.18 }}>
          ...
          {activeTab === 'overview' && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <div className="bg-[#1E1E1E] border border-[#333333] rounded-lg p-5">
                <SectionHeader icon={FileText} title="About Me" subtitle="Short bio about yourself"
                  action={<button onClick={() => setEditSection(editSection === 'bio' ? null : 'bio')} className="p-1.5 rounded-md bg-[#2C2C2C] text-[#9CA3AF] hover:text-white transition-colors"><Pencil className="w-3.5 h-3.5" /></button>} />
                {editSection === 'bio' ? (
                  <div className="space-y-3">
                    <input value={draft.headline} onChange={e => setDraft(d => ({ ...d, headline: e.target.value }))} placeholder="Your headline (e.g. Full Stack Developer | Open to Work)"
                      className="w-full px-3 py-2 bg-[#141414] border border-[#333333] rounded-md text-xs text-[#F4F4F5] placeholder-[#6B7280] focus:outline-none focus:border-[#EA5D3A] transition-all" />
                    <textarea value={draft.bio} onChange={e => setDraft(d => ({ ...d, bio: e.target.value }))} placeholder="Write a short bio..." rows={4}
                      className="w-full px-3 py-2 bg-[#141414] border border-[#333333] rounded-md text-xs text-[#F4F4F5] placeholder-[#6B7280] focus:outline-none focus:border-[#EA5D3A] transition-all resize-none" />
              ...
              <div className="bg-[#1E1E1E] border border-[#333333] rounded-lg p-5">
                <SectionHeader icon={Mail} title="Contact & Links" subtitle="Social profiles and contact info"
                  action={<button onClick={() => setEditSection(editSection === 'contact' ? null : 'contact')} className="p-1.5 rounded-md bg-[#2C2C2C] text-[#9CA3AF] hover:text-white transition-colors"><Pencil className="w-3.5 h-3.5" /></button>} />
                {editSection === 'contact' ? (
                    ...
                    {[
                      ...
                      <div key={key} className="relative">
                        ...
                        <input value={draft[key] || ''} onChange={e => setDraft(d => ({ ...d, [key]: e.target.value }))} placeholder={placeholder}
                          className="w-full pl-9 pr-3 py-2 bg-[#141414] border border-[#333333] rounded-md text-xs text-[#F4F4F5] placeholder-[#6B7280] focus:outline-none focus:border-[#EA5D3A] transition-all" />
                      </div>
                    ))}
              ...
              <div className="bg-[#1E1E1E] border border-[#333333] rounded-lg p-5 md:col-span-2">
                <SectionHeader icon={Code2} title="Skills & Technologies" subtitle="Languages, frameworks and tools you know"
                  action={<button onClick={() => setEditSection(editSection === 'skills' ? null : 'skills')} className="p-1.5 rounded-md bg-[#2C2C2C] text-[#9CA3AF] hover:text-white transition-colors"><Plus className="w-3.5 h-3.5" /></button>} />
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <button onClick={() => addItem('projects', { title: '', description: '', tech: [], github: '', demo: '', status: 'In Progress' })}
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-md bg-[#262626] border border-[#333333] text-[#9CA3AF] hover:text-white text-xs font-medium transition-all">
                  <Plus className="w-3.5 h-3.5" /> Add Project
                </button>
              </div>
              {draft.projects.length === 0 && <div className="bg-[#1E1E1E] border border-[#333333] rounded-lg"><EmptyState message="Showcase your projects to stand out to recruiters" /></div>}
          ...
          {activeTab === 'experience' && (
            <div className="space-y-4">
              ...
              {draft.experience.length === 0 && <div className="bg-[#1E1E1E] border border-[#333333] rounded-lg"><EmptyState message="Add internships, full-time roles, freelance work etc." /></div>}
          ...
          {activeTab === 'education' && (
            <div className="space-y-4">
              ...
              {draft.education.length === 0 && <div className="bg-[#1E1E1E] border border-[#333333] rounded-lg"><EmptyState message="Add your college, school, bootcamp or courses" /></div>}
          ...
          {activeTab === 'achievements' && (
            <div className="space-y-4">
              ...
              {draft.achievements.length === 0 && <div className="bg-[#1E1E1E] border border-[#333333] rounded-lg"><EmptyState message="Add hackathon wins, competition ranks, open source contributions, etc." /></div>}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
