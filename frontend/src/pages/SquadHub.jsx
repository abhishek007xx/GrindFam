...
export default function SquadHub() {
  ...
  // Compute user streak from solved dates
  useEffect(() => {
  ...
  return (
    <div className="h-full flex-1 flex overflow-hidden bg-[#0d1117] text-[#dce5d9]" style={{ fontFamily: "'Inter', sans-serif" }}>
      {/* COLUMN 1: Server / Community Rail (72px, bg #091009) */}
      <div className="w-[72px] max-md:w-14 bg-[#091009] flex flex-col items-center py-3 gap-1 flex-shrink-0 border-r border-[#21262d] overflow-y-auto scrollbar-hide">
        ...
        {mySquads.map((sq) => (
          <SquadIcon
            key={sq.id}
            squad={sq}
            isActive={!activeDMThread && activeSquad?.id === sq.id}
                ...
                <div>
                  ...
                  {TEXT_CHANNELS.map((ch) => (
                    <ChannelButton
                      ...
                      icon={ch.icon}
                      label={ch.label}
                      isActive={!activeDMThread && activeChannel === ch.id}
      ...
      {/* COLUMN 3: Main Content Area (flex-1, bg #111315) */}
        ...
        {activeDMThread ? (
          ...
          <>
            ...
            <div className="flex-1 flex min-h-0">
              ...
              {/* COLUMN 4: Member List Sidebar (240px, bg #161b22) */}
              {showMemberList && (
