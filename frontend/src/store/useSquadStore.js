...
export const useSquadStore = create((set, get) => ({
  ...
  // Load all squads current user is member of
  loadMySquads: async () => {
    ...
    try {
      ...
      // Fetch user's squad_member entries
      ...
      // Automatically set active squad if none selected or invalid
      const currentActive = get().activeSquad;
      if (!currentActive || !enrichedSquads.some(s => s.id === currentActive.id)) {
        if (enrichedSquads.length > 0) {
          get().setActiveSquad(enrichedSquads[0].id);
        }
      }
    } catch (err) {
  ...
  // Discover public community squads
  fetchCommunitySquads: async () => {
    try {
      const { data: squads, error } = await supabase
        .from('squads')
        .select('*')
        .eq('squad_type', 'community')
        .order('created_at', { ascending: false });

      if (error) throw error;

      // Fetch member counts for each
    ...
    try {
      ...
      if (userIds.length > 0) {
        const { data: profiles } = await supabase
          .from('profiles')
          .select('id, username, leetcode_username, discord_username')
          .in('id', userIds);
      ...
      const enrichedSnippets = (rawSnippets || []).map(s => ({
        ...s,
        author: {
          name: snipProfileMap[s.user_id]?.username || snipProfileMap[s.user_id]?.leetcode_username || 'Member'
        }
      }));
  ...
  // Join squad by invite code or raw UUID
  joinByCode: async (codeOrId) => {
    ...
    // Find squad by invite_code or code or id
    let targetSquad = null;
    const { data: byInvite } = await supabase
      .from('squads')
      .select('*')
      .ilike('invite_code', cleanInput)
      .maybeSingle();
    ...
    // Insert user into squad_members
  ...
  // Send message directly via Supabase client
  ...
  // Realtime subscription with channel cleanup preventing leaks
  subscribeRealtime: (squadId) => {
    get().unsubscribeRealtime();

    const channel = supabase
      .channel(`squad-realtime-${squadId}`)
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'squad_messages', filter: `squad_id=eq.${squadId}` }, async (payload) => {
        const newMsg = payload.new;
        const { data: prof } = await supabase.from('profiles').select('username, leetcode_username').eq('id', newMsg.user_id).maybeSingle();
